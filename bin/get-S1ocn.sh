...
cd /home/users/smartmet/data/sen3
...
query="https://finder.creodias.eu/resto/api/collections/Sentinel3/search.json?maxRecords=2000&startDate=${sdate}T00:00:00Z&completionDate=${edate}T23:59:59Z&timeliness=Short+Time+Critical&processingLevel=LEVEL2&productType=SY_2_V10___&geometry=POLYGON((-80+-10,-80+-60,-50+-60,-50+-10,-80+-10))&sortParam=startDate&sortOrder=descending&status=all&dataset=ESA-DATASET"
echo $query
...
echo "$d SEN3 SYN combination:" `ls -d S3*.SEN3`
parallel cdo -s -O -f grb2 -b P12 ensmean -{1} S3A_SY_2_V10____${d}T*.SEN3/'{=1 s:.*selname,:: =}'.nc\
 -{1} S3B_SY_2_V10____${d}T*.SEN3/'{=1 s:.*selname,:: =}'.nc\
 ../grib/S3SY_${d:0:4}0101T000000_${d}T000000_'{=1 s:.*selname,:: =}'.grib :::: sen3codes.lst && rm -rf S3?_SY_2_V10____*.SEN3
parallel grib_set -r -s centre=97,dataDate=$d ../grib/S3SY_${d:0:4}0101T000000_${d}T000000_{}.grib\
