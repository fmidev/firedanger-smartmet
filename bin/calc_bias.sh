...
cdo --eccodes -O sub \
    ...
    -ymonmean -remapbil,${otags[0]}-sam-grid -selvar,$unbound $modpath \
...
cdo --eccodes -O div \
    ...
    -ymonmean -remapbil,${otags[0]}-sam-grid -chname,10si,ws -selvar,10si $modpath \
    ${prefix}_ws_bias_$ending &
# division method for tp- and erate variables
cdo --eccodes -O div \
    -ymonmean -selvar,tp $obspath \
    -ymonmean -remapbil,${otags[0]}-sam-grid -expr,'tp=tprate*3600*24;tp=((tp<0.0001)?0:tp);' -selvar,tprate $modpath \
    ${prefix}_tp_bias_$ending &
cdo --eccodes -O div \
    -ymonmean -selvar,e $obspath \
    -ymonmean -remapbil,${otags[0]}-sam-grid -expr,'e=erate*3600*24;' -selvar,erate $modpath \
    ${prefix}_e_bias_$ending &
# VARIANCE division also for unbounded variables
cdo --eccodes -O div  \
    -ymonvar1 -selvar,$unbound $obspath \
    -ymonvar1 -remapbil,${otags[0]}-sam-grid -selvar,$unbound $modpath \
    ${prefix}_unbound_varc_$ending &
# division method for bounded variables
cdo --eccodes -O div \
    -ymonvar1 -expr,'ws=sqrt(10u^2+10v^2)' -selvar,10u,10v $obspath \
    -ymonvar1 -remapbil,${otags[0]}-sam-grid -chname,10si,ws -selvar,10si $modpath \
    ${prefix}_ws_varc_$ending &
# division method for tp- and erate variables
cdo --eccodes -O div \
    -ymonvar1 -selvar,tp $obspath \
    -ymonvar1 -remapbil,${otags[0]}-sam-grid -expr,'tp=tprate*3600*24;tp=((tp<0.0001)?0:tp);' -selvar,tprate $modpath \
    ${prefix}_tp_varc_$ending &
cdo --eccodes -O div \
    -ymonvar1 -selvar,e $obspath \
    -ymonvar1 -remapbil,${otags[0]}-sam-grid -expr,'e=erate*3600*24' -selvar,erate $modpath \
