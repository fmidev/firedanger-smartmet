...
[ -f ec-sf-$year$month-all-24h-$abr.grib ] && echo "SF Data file already downloaded" || /home/users/smartmet/firedanger-smartmet/bin/cds-sf-all-24h.py $year $month $area $abr
[ -f ec-sf-$year$month-pl-12h-$abr.grib ] && echo "SF pressurelevel Data file already downloaded" || /home/users/smartmet/firedanger-smartmet/bin/cds-sf-pl-12h.py $year $month $area $abr
[ -f ecsf_$year-$month-01_$abr-swvls.grib ] && echo "SF SoilLevel Data already downloaded" || sed s:2023-01-01:$year-$month-01:g /home/users/smartmet/mars/seas-swvl.mars | /home/users/smartmet/firedanger-smartmet/bin/mars


# ensure new eccodes and cdo
[ -f ens/ec-sf_$year${month}_all-24h-$abr-50.grib ] && echo "Ensemble member sl files ready" || grib_copy ec-sf-$year$month-all-24h-$abr.grib ens/ec-sf_$year${month}_all-24h-$abr-[number].grib
...
[ -f ens/ec-sf_$year${month}_all-24h-$abr-50.grib ] && ! [ -f ens/ec-${bsf}_$year${month}_unbound-24h-$abr-50.grib ] && \
 seq 0 50 | parallel cdo -s -b P8 -O --eccodes ymonadd \
    ...
    -selname,2d,2t,stl1 $era/$era-ecsf_2000-2020_unbound_bias_$abr.grib \
...
[ -f ens/ec-sf_$year${month}_all-24h-$abr-50.grib ] && ! [ -f ens/ec-${bsf}_$year${month}_snow-24h-$abr-50.grib ] && \
 seq 0 50 | parallel cdo -s -O -b P12 --eccodes setmisstoc,0.0 -aexprf,ec-sde.instr -ymonadd \
    ...
    -selname,rsn,sd $era/$era-ecsf_2000-2020_unbound_bias_$abr.grib \
...
[ -f ens/ec-sf_$year${month}_all-24h-$abr-50.grib ] && ! [ -f ens/ec-${bsf}_$year${month}_bound-24h-$abr-50.grib ] && \
 seq 0 50 | parallel -q cdo -s -b P8 -O --eccodes ymonmul \
    ...
    -aexpr,'10u=ws;10v=ws;' -selname,ws $era/$era-ecsf_2000-2020_bound_bias_$abr.grib \
...
[ -f ens/ec-sf_$year${month}_all-24h-$abr-50.grib ] && ! [ -f ens/ec-${bsf}_$year${month}_acc-24h-$abr-50.grib ] && \
 seq 0 50 | parallel "cdo -s --eccodes -O mergetime -seltimestep,1 -selname,e,tp ens/ec-sf_$year${month}_all-24h-$abr-{}.grib \
     -deltat -selname,e,tp ens/ec-sf_$year${month}_all-24h-$abr-{}.grib disacc-tmp-{}.grib && \
    cdo -s --eccodes ymonmul -remap,$era-$abr-grid,ec-sf-$era-$abr-weights.nc disacc-tmp-{}.grib \
     -selname,e,tp $era/$era-ecsf_2000-2020_bound_bias_$abr.grib \
     ens/ec-${bsf}_$year${month}_disacc-24h-$abr-{}.grib && \
    cdo -s --eccodes -b P8 timcumsum ens/ec-${bsf}_$year${month}_disacc-24h-$abr-{}.grib ens/ec-${bsf}_$year${month}_acc-24h-$abr-{}.grib" \
...
wait
rm ens/ec-${bsf}_$year${month}_*-24h-$abr-*.grib
